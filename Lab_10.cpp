...
//Реализация четния типа вагона из текстового файла (0, 1, 2)
void read_file(const char* filename) {
    ...
    if (!fin.is_open()) {
        cout << "Ну удалось открыть файл!" << endl;
        return;
    }

    while (fin >> marshryt >> tip_value) {
        found = false;
        for (int i = 0; i < SIZE; i++) {
            if (strcmp(marshryt, trains[i].marshryt) == 0) {
                trains[i].tip_train = (tip)tip_value;
                found = true;
                break;
            }
        }
        if (!found) {
            cout << "Маршрут '" << marshryt << "' не найден" << endl;
        }
    }

    fin.close();
    cout << "Данные о типах поездов загружены из файла" << endl;
}

// Подпрограмма для записи массива структур в бинарный файл
void save_binfile() {
    ofstream out("Lab_10.bin", ios::binary | ios::out);
    out.write((char*)trains, sizeof(train) * SIZE);
    out.close();
    cout << "\nДанные успешно сохранены в бинарный файл " << "Lab_10.bin" << endl;
}

// Подпрограмма для чтения массива структур из бинарного файла
void load_binfile() {
    ifstream in("trains.bin", ios::binary | ios::in);
    in.read((char*)trains, sizeof(train) * SIZE);
    in.close();
}
...
int main() {
    ...
    load_binfile();
    cout << "\nДанные после чтения из бинарного файла" << endl;
    cout << "Название маршрута: " << trains[0].marshryt << "\n";
    cout << "Количество вагонов: " << trains[0].count_vagon << "\n";
    cout << "Пункт отправления: " << trains[0].pynkt_otpav << "\n";
    cout << "Пункт прибытия: " << trains[0].pynkt_pribit << "\n";
    cout << "Тип поезда: " << tipname[trains[0].tip_train] << "\n";
